...
const searchSchema = {
  ...
  categories: z
    ...
    .default('general')
    .describe(
      'SearXNG search category. Options: general, news, images, videos, science, files, it, map.'
    ),
  time_range: z
    .string()
    .optional()
...
export function registerSearchTool(server: McpServer, config: Config): void {
  ...
  server.tool(
    'web_search',
    'Search the web using a local SearXNG instance. Returns titles, URLs, and snippets for each result. Use this to find current information, documentation, articles, and more.',
    searchSchema,
    async ({ query, count = 10, categories = 'general', time_range, language = 'en' }) => {
      // Validate inputs
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                query: query.trim(),
                hint:
                  message.includes('unavailable') || message.includes('ECONNREFUSED')
