...
import type { Config, FetchResult } from '../types.js';
...
const fetchSchema = {
  ...
  extract_mode: z
    ...
    .default('text')
    .describe(
      "How to process the page. 'text' extracts readable content (default). 'raw' returns raw HTML. 'markdown' converts to markdown."
    ),
  max_length: z
    .number()
    .optional()
...
export function registerFetchTool(server: McpServer, config: Config): void {
  ...
  server.tool(
    'web_fetch',
    'Fetch a web page and extract its readable content. Use after web_search to get full article text from a URL. Supports HTML pages and PDFs.',
    fetchSchema,
    async ({
      url,
      extract_mode = 'text',
      max_length = config.fetch.max_content_length,
      timeout_seconds = config.fetch.default_timeout_seconds,
    }) => {
      // Validate URL
