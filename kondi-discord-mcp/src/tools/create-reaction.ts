...
const createReactionSchema = {
  ...
  emoji: z
    .string()
    .describe(
      'The emoji to react with. Use Unicode emoji (e.g., "\u{1F44D}") or custom emoji format "name:id".'
    ),
};

export function registerCreateReactionTool(server: McpServer, config: Config): void {
  const client = new DiscordClient(config);

  server.tool(
    'discord_create_reaction',
    'Add a reaction emoji to a message in a Discord channel. Supports Unicode emoji and custom guild emoji.',
    ...
    async ({ channel_id, message_id, emoji }) => {
      ...
      try {
        await client.createReaction(channel_id, message_id, emoji);
