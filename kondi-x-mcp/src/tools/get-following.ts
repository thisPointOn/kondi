...
const getFollowingSchema = {
  user_id: z
    .string()
    .describe('The numeric ID of the user whose following list to retrieve.'),
  max_results: z
    .number()
    .min(1)
    .max(1000)
    .optional()
    .default(100)
    .describe('Maximum number of followed users to return. Default: 100. Min: 1, Max: 1000.'),
  user_fields: z
    .string()
    .optional()
    .describe(
...
export function registerGetFollowingTool(server: McpServer, config: Config): void {
  ...
  server.tool(
    ...
    async ({ user_id, max_results = 100, user_fields }) => {
      ...
      try {
        const response = await client.getFollowing(user_id, sanitizedMaxResults, user_fields);
        ...
        const message = error instanceof Error ? error.message : 'Unknown error getting following list';
